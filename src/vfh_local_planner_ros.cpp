...
namespace vfh_local_planner
{
    ...
    //     if (!base_local_planner::transformGlobalPlan(*tf_, global_plan_, current_pose_, *costmap_, global_frame_, transformed_plan)) {
    //         ROS_WARN("Could not transform the global plan to the frame of the controller");
    //         return false;
    //     }
    //     //now we'll prune the plan based on the position of the robot
    //     if(prune_plan_)
    //         base_local_planner::prunePlan(current_pose_, transformed_plan, global_plan_);
        ...
        if (!transformed_plan.empty())
        {
            ...
            int point_index = GetPlanPoint(transformed_plan, global_plan_,current_pose_);
        ...
        if (base_local_planner::getGoalPositionDistance(current_pose_, goal_x, goal_y) <= config_.xy_goal_tolerance_ || xy_goal_latch_)
        {
            ...
            if (fabs(base_local_planner::getGoalOrientationAngleDifference(current_pose_, goal_th)) <= config_.yaw_goal_tolerance_)
            {
                ...
                vfh_planner.RotateToGoal(current_pose_, current_vel, goal_th, cmd_vel);
                return true;
            }
        }       
        //#############################################################################################################################################
        ...
        {
            double global_plan_goal_angle = atan2((global_goal.getOrigin().getY()-current_pose_.getOrigin().getY()), (global_goal.getOrigin().getX()-current_pose_.getOrigin().getX()));
            goal_distance = 0.2;
            if (vfh_planner.DirectionIsClear(global_plan_goal_angle))
            {
                ...
                goal_distance = std::min(sqrt(pow((global_goal.getOrigin().getX()-current_pose_.getOrigin().getX()),2)+pow((global_goal.getOrigin().getY()-current_pose_.getOrigin().getY()),2)), 0.3);
            }
            else
            {
        ...
        {
            double intermediary_goal_orientation = atan2((intermediary_goal_point.getOrigin().getY()-current_pose_.getOrigin().getY()), (intermediary_goal_point.getOrigin().getX()-current_pose_.getOrigin().getX()));
            ...
            else
            {
                ...
                goal_distance = sqrt(pow((intermediary_goal_point.getOrigin().getX()-current_pose_.getOrigin().getX()),2)+pow((intermediary_goal_point.getOrigin().getY()-current_pose_.getOrigin().getY()),2));
            }
        }
        ...
        if (rotating_to_goal_)
        {
            ...
            vfh_planner.RotateToGoal(current_pose_, current_vel, direction_to_follow, cmd_vel);
            if (fabs(base_local_planner::getGoalOrientationAngleDifference(current_pose_, direction_to_follow)) < config_.yaw_goal_tolerance_)
            {
                rotating_to_goal_ = false;
            }
            return true;
        ...
        else if (fabs(base_local_planner::getGoalOrientationAngleDifference(current_pose_, direction_to_follow)) > M_PI/3)
        {
            std::cout << "rotating to goal to start" << std::endl;
            rotating_to_goal_ = true;
        ...
        else 
        {
            ...
            vfh_planner.DriveToward(angles::shortest_angular_distance(tf::getYaw(current_pose_.getRotation()),direction_to_follow), goal_distance, cmd_vel);
        }
    ...
    int VFHPlannerRos::GetPlanPoint(std::vector<geometry_msgs::PoseStamped> transformed_plan, std::vector<geometry_msgs::PoseStamped> &global_plan, tf::Stamped<tf::Pose> current_pose_)
    {
        int point = 0;
        for (int i = 0; i < transformed_plan.size(); i++)
        {
            double point_distance = sqrt(pow((transformed_plan.at(i).pose.position.x-current_pose_.getOrigin().getX()),2)+pow((transformed_plan.at(i).pose.position.y-current_pose_.getOrigin().getY()),2));
